Here are all the placeholders in angle brackets (`< >`) that need to be replaced with your specific values:
...
- `<your_gcs_bucket>`: Name of your Google Cloud Storage (GCS) bucket.
...
with DAG(
    ...
    sftp_sensor >> download_task >> upload_to_gcs_landing_task >> load_raw_bq_task >> get_years_task >> generate_tasks
    generate_tasks >> delete_raw_task >> archive_file_task
```

Replace the placeholders with the appropriate values for your setup. This ensures the DAG processes the data correctly and handles file management and cleanup appropriately.